...
class EBUSClassificationPipeline(DatasetPipeline):
    ...
    def __init__(self, image_shape=(256, 256), class_config_nbr=3, *args, **kwargs):
        ...
        self.set_stations_config(config_nbr=class_config_nbr)
        self.num_classes = self.get_num_stations()
    ...
    def loader_function(self, filepath, index=None, *args, **kwargs):
        ...
        targets = np.zeros(shape=self.num_classes, dtype=np.uint8) # [0 0 0 0 0 0 0] for LABEL_CONFIG=3
        targets[self.get_stations_config(station_label)] = 1 #[0 0 0 0 0 1 0] for station_name = 10L
    ...
    def get_stations_config(self, label):
        try:
    ...
    def set_stations_config(self, config_nbr):
        if config_nbr == 1:
